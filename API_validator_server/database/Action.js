...
const updateAction = async (conn, action_id, pass, fail) => {
    try {
        // action 업데이트 후 id 반환
...
const getActionsByDate = async (conn, startTime, endTime) => {
  try {
      // action 업데이트 후 id 반환
      let sql = `SELECT action_id, pass_cnt, fail_cnt, DATE_FORMAT(from_unixtime(created_at), '%Y-%m-%d %H:%i:%s') as 'created_at' FROM tbl_github_actions 
