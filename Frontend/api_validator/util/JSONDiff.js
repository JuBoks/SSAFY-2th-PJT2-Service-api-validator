...
function JSONDiff(props) {
  ...
  useLayoutEffect(() => {
    ...
    const barsegment = elementsRef.current.map((element, index) => {
      ...
      const max = textHeight > containerRect.height ? containerRect.height - 36 : textHeight;
  ...
  const html = lines.map((line, index) => {
    if (line.startsWith("-")) {
      return <p key={index} className={styles.minus} ref={elementsRef.current[index]}>{line}</p>;
    }
    else if (line.startsWith("+")){
      return <p key={index} className={styles.plus} ref={elementsRef.current[index]}>{line}</p>;
    } 
    else {
...
return (
    <div className={styles.bigcontainer}>
      <div className={styles.container} ref={containerRef}>
        <div className={styles.text} ref={textRef}>{html}</div>
      </div>
